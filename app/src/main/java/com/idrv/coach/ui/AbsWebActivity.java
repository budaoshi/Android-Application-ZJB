 ...
 * @author sunjianfei
 */
public abstract class AbsWebActivity<ViewModel> extends BaseActivity<ViewModel> {
    ...
    protected void onPause() {
        if (null != mWebView) {
            if (mParamBuilder.isNeedReload()) {
                mWebView.reload();
            }
            mWebView.onPause();
            UIHelper.hideSoftInput(mWebView);
        }
        super.onPause();
    }
    ...
    private void disableShareJs(boolean disable) {
        mToolbarLayout.setRightIconShowStatus(disable);
    }
    ...
    private void shareJs(WebShareBean shareBean) {
        ...
        //显示右上角的分享按钮
        disableShareJs(false);
        mToolbarLayout.setRightIcon(R.drawable.icon_news_share);
